    ...
    for user in users:
        ...
        if current_year_birthday < current_date:
           ...
           # тому залишила дату наступного року при виведенні
            ...
            #urrent_year_birthday_formatted = current_year_birthday.strftime("%Y.%m.%d")
            ...
            if birthday_week_day == 6:
                ...
                new_congratulation_date2 = (current_year_birthday + timedelta(days=1)).strftime("%Y.%m.%d")
                new_user_data = {"name": user["name"], "congratulation_date": new_congratulation_date2}
                next_birthdays.append(new_user_data)
